...
| **the chords** | a new validator leads the network every 4 beats (one bar); the harmony moves with the leader schedule — and the melody sits where the leader stands, panning across the stereo stage as the schedule rotates (the same position the visual spotlight jumps to). When a giant leads (an enormous stake), its chord carries a root an octave deeper. |
| **a great bell** | the slot counter crossing a million — watch the HUD's number roll over as it tolls (~every 4½ days). |
